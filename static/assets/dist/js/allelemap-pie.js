...
// Código NUEVO (escala log + marcas nativas)
$(function () {
    const ACTUAL_MIN = 1;
    ...
    const TICKS_REAL = [1, 4000000];
    const TICKS_LOG = TICKS_REAL.map(v => Math.log10(v));
    const TICKS_LABELS = ['1', '4M'];
    ...
    $slider.on('slide', function(ev) { updateLabel(ev.value); });
    ...
    window.getSampleRange = function() {
        var vals = $slider.slider('getValue');
        return { min: logToReal(vals[0]), max: logToReal(vals[1]) };
    };
});
                    ...
                    function LoadAlleleGroup() {
                        ...
                        axios
                            ...
                            .then(function (response) {
                                ...
                                if (kind_of_info == 'Primary') {
                                    kind_of_info_key = 'P'
                                } else if (kind_of_info == 'Secondary') {
                                    kind_of_info_key = 'S'
                                } else if (kind_of_info == 'Both') {
                                    kind_of_info_key = 'both'
                                }
                                    ...
                                    <filter id="shadow-${index}" x="-20%" y="-20%" width="150%" height="150%">
                                        <feDropShadow dx="1.5" dy="1.5" stdDeviation="1.2" flood-color="rgba(0,0,0,0.4)" />
                                            ...
                                            if ((isCountry) && (kind_of_info_key == 'P')) {
                                                ...
                                                const mapObj = $('#map').vectorMap('get', 'mapObject');
                                                mapObj.setFocus({
                                                    x: ((aLong + 180) / 360) - 0.02,
                                                    y: (Math.abs(((aLat + 90) / 180) - 1) + 0.1),
                                                    scale: 4,
                                                    animate: true
                                                });
                                            }
