...
function renderDynamicTimeline(type, data) {
    ...
    function scrollToLastTimelineItem() {
        ...
        if (timelineItems.length > 0) {
            ...
            if (targetIndex >= 0) {
                timelineItems[targetIndex].scrollIntoView({ behavior: "smooth", block: "center" });
            }
        }
    }
    ...
    function renderItems(upToIndex) {
        ...
        let controls = `
            <div class="d-flex justify-content-between mt-2">
                <div>
                    <button id="prevTimelineItemBtn" class="btn btn-secondary" ${upToIndex === 0 ? "disabled" : ""}>Previous</button>
                </div>
                <div>
                    <button id="playTimelineBtn" class="btn btn-success">${isPlaying ? "Pause" : "Play"}</button>
                </div>
                <div>
                    <button id="nextTimelineItemBtn" class="btn btn-primary" ${upToIndex === items.length - 1 ? "disabled" : ""}>Next</button>
                </div>
            </div>
        `;
