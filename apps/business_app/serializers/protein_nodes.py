...
class ProteinNodeSerializer(serializers.ModelSerializer):
    """Serializer for ProteinNode model with graph-aware fields."""
    ...
    def _get_graph_info(self, obj, function_to_call):
        ...
        Args:
            obj: The ProteinNode instance
            function_to_call: Function to extract tree data (extract_parents_tree or extract_children_tree)

        Returns:
            set: Set of node numbers in the graph, or empty list if graph not available
        """
    ...
    def get_sucessors(self, obj):
        ...
        cache_key = ProteinNode.CACHE_KEY_SUCESSORS.format(
            uploaded_file_id=obj.uploaded_file_id, number=obj.number
        )
        if not cache.has_key(cache_key):
