        ...
        try:
            ...
            if not gen:
                ...
                return Response({
                    ...
                    "suggestions": list(similar_genes) if similar_genes else ["Verifica el nombre del gen"]
                }, status=status.HTTP_404_NOT_FOUND)
            
            # OPCIÓN 1: Usando ORM con values() (más rápido que serializer)
            caracteristicas = CaracteristicaGen.objects.filter(
                gen=gen
            ).select_related('gen').values(
                'id',
                'gen_id',
                'archivo_origen',
                'gene',
        ...
        try:
            ...
            def generate_json():
                ...
                caracteristicas = CaracteristicaGen.objects.filter(
                    ...
                    'id', 'gen_id', 'archivo_origen', 'gene', 'cord', 
                    'valor', 'color', 'protein', 'alleleasoc', 
                    'species', 'fecha_creacion'
            ...
            response['Content-Disposition'] = f'attachment; filename="{gene_code}_data.json"'
            return response
